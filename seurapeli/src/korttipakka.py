...
class Card:
    ...
    def draw(self, surface, pos):  # surface on näyttö , jolle kortti vedetään
        ...
        # tuple, joka sisältää koordinaatit kortin vasemmasta yläkulmasta
        self.rect.topleft = pos
        # rect kertoo mihin näytöllä se kortti vedetään
...
class Deck:  # korttipakka
    ...
    def draw(self, surface, pos):  # piirtää koko pakan
        a = 650
        i = 0
        # i on indeksi (kuinka mones) mutta loopissa toimitaan tuplejen kanssa #i,
    ...
    def deal(self):
        # poistaa kierroksella jaetun kortin (tähän pitää myös saada aikaiseksi se, että niitä on kolme)
